...
**参考**: A-Evolve(循环范式) / self-evolving-agent(能力状态机) / Darwin(Critic评估) / agent-seed(大小约束) / solo-founder(四步循环) / mattpocock/skills(对齐协议) / agentmemory(Hook+变更影响) / financial-services(自包含规则)
...
| 改进 | 来源 | 解决的问题 |
|------|------|-----------|
| grill-me 对齐协议 | mattpocock/skills | v3 直接执行,缺少"对齐"环节,容易偏 |
| Hook 生命周期增强 | agentmemory | v3 hooks 只有 bootstrap/teardown,缺少工具级上下文注入 |
| 变更影响清单 | agentmemory | v3 修改核心组件时无同步检查,容易遗漏 |
| CONTEXT.md 术语表 | mattpocock/skills | v3 术语不统一,存在歧义 |
| ADR 架构决策记录 | mattpocock/skills | v3 重要决策未记录,容易重复犯错 |
| Skill 自包含规则 | financial-services | v3 部分 Skill 依赖外部路径,可移植性差 |
...
**触发条件**：用户说“grill me”、“对齐”、“压力测试”、“审视”
**规则**：
1. 一次只问一个问题，等用户回答后再问下一个
2. 每个问题提供推荐答案，但让用户决定
3. 如果问题可以通过读文件/查记忆回答，直接探索，不问用户
4. 追问方向：边界、依赖、失败场景、替代方案、术语精确性
...
**集成位置**：`skills/grill-me/SKILL.md`

### 9.2 变更影响清单

**规则**：修改核心组件时，必须同步检查 docs/change-impact-checklist.md 中对应清单

| 修改对象 | 必须同步的文件 |
|---------|-------------|
| Skill | SKILL.md + CONTEXT.md + MEMORY.md + work-tools.md |
| Memory 架构 | AGENTS.md + CONTEXT.md + docs/adr/ + EVOLUTION-PROTOCOL.md |
| Evolution 引擎 | EVOLUTION-PROTOCOL.md + patterns.json + failures.json + AGENTS.md + docs/adr/ |
| 发布流程 | TOOLS.md + work-tools.md + daily/ + 相关 Skill |
| 安全规则 | SOUL.md + AGENTS.md + RULES.md |

**集成位置**：`docs/change-impact-checklist.md` + `hooks/hooks.yaml` change_impact 钩子

### 9.3 ADR 架构决策记录

**规则**：重要架构决策必须记录为 ADR，格式：
- 状态：已接受/已废弃/已替代
- 背景：为什么需要这个决策
- 决策：具体方案
- 理由：为什么选这个方案
